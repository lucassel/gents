...
| *two unifying theories for allez* | **it stays a list** | First: "one movement, giving in" — knocked over by *allez?!*, where nothing is conceded. Then: "two camps sorted by position" — knocked over by *alsge wilt dat gedaan is* and by encouragement, neither of which is accepting or refusing. Recorded because the failure is the lesson: the pull to make a tidy system out of a particle is exactly what produces confident wrong grammar. The French imperative reading (*aller*, a push) is kept as a memory aid, never as a generator. |
